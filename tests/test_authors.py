...
async def fixed_authors(
    get_test_db_session: AsyncSession, clear_authors: None
) -> List[Author]:
    fixed_authors = [
...
async def test_get_authors_filtered_200(
    ...
    list_validate_response = [
        AuthorResponse.model_validate(author) for author in response.json()
    ]

    assert len(list_validate_response) == len(expected_names)

    sorted_response_names = sorted(
        [author.full_name for author in list_validate_response]
    )
    sorted_expected_names = sorted(
        author.full_name
        for author in fixed_authors
        if author.full_name in expected_names
    )

    assert sorted_response_names == sorted_expected_names

    expected_authors_from_db = [
        author for author in fixed_authors if author.full_name in expected_names
    ]
...
async def test_get_author_detail_200(
    client: AsyncClient, fixed_authors: list[Author], get_test_db_session: AsyncSession
):
...
@pytest.mark.asyncio
async def test_get_author_detail_not_found_404(client: AsyncClient):
    response = await client.get("/authors/9999999")
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_create_author_anonymous_return_401(
    client: AsyncClient, get_test_db_session: AsyncSession, setup_auth
):
...
async def test_create_author_success_201(
    ...
    auth_behavior = {"id": 1, "role": "admin", "exp": 1234, "token_str": "access_token"}
    setup_auth(auth_behavior)

    author_payloads = {"full_name": "a1", "country": "c1", "birth_date": "1978-05-10"}
    ...
    get_test_db_session.expire_all()
    result = await get_test_db_session.execute(
        select(Author).where(Author.id == response_validated_data.id)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"country": "RU", "birth_date": "1980-01-01"}, ("full_name",), ("missing",)),
        ...
        ({"full_name": "Viktor Blood", "country": "RU"}, ("birth_date",), ("missing",)),
    ],
)
@pytest.mark.asyncio
async def test_create_author_invalid_payloads_422(
    ...
    for i, loc_name in enumerate(expected_errors_loc):
        error_found = any(
            loc_name in err["loc"] and expected_errors_type[i] == err["type"]
            for err in error_data
        )

        assert (
            error_found
        ), f"Error not found for field {loc_name} with type {expected_errors_type[i]}"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, {"full_name": ""}, 422, ("full_name",), ("string_too_short",)),  # <1
        # Автор существует, поле full_name не валидно
        (
            True,
            {"full_name": "a" * 35},  # >30
        ...
        (True, {"country": ""}, 422, ("country",), ("string_too_short",)),  # <1
...
async def test_path_author_invalid_payload_and_not_found_422_404(
    ...
    if use_valid_id:
        ...
        for i, loc_name in enumerate(expected_err_loc):
            error_found = any(
                loc_name in err["loc"] and err["type"] == expected_err_type[i]
                for err in errors
            )
...
async def test_patch_author_success_200(
    ...
    for key in patch_payload.keys():
        ...
        assert (
            pydantic_val == patch_payload[key]
    ...
    for field_name in AuthorResponse.model_fields.keys():
        ...
        assert (
            db_val == pydantic_val
        ), f"Поле {field_name} не совпадает: БД: {db_val}, Ответ: {pydantic_val}"
