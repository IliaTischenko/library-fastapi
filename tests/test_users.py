...
async def clear_users(get_test_db_session: AsyncSession) -> AsyncGenerator[None, None]:
    yield
...
async def fixed_users(
    get_test_db_session: AsyncSession, clear_users: None
) -> list[User]:
    users = [
        User(
            username="username1",
            hashed_pass=get_password_hash("correct_password"),
            role=UserRole.ADMIN,
        ),
        User(
            username="username2",
...
async def test_get_users_success_200(
    client: AsyncClient, setup_auth, fixed_users: list[User]
):
...
async def test_register_admin_success_201(
    client: AsyncClient, setup_auth, clear_users, get_test_db_session: AsyncSession
):
    auth_behavior = {"id": 1, "role": "admin", "exp": 123, "token_str": "at"}
    setup_auth(auth_behavior)

    admin_payload = {"username": "username_1", "password": "12345678"}
...
@pytest.mark.parametrize(
    ...
    (
        ...
        # username >24
        (
            {"username": "g" * 25, "password": "correct_pass"},
            "username",
            "string_too_long",
        ),
        # password пустой/<8
        (
            {"username": "correct_username", "password": ""},
            "password",
            "string_too_short",
        ),
        # password >66
        (
            {"username": "correct_username", "password": "f" * 66},
            "password",
            "string_too_long",
        ),
...
async def test_register_reader_success_201(
    ...
    query = (
        ...
        .options(selectinload(Reader.user), selectinload(Reader.books))
    )
...
async def test_patch_user_success_200(
    ...
    user_payload = {"old_password": "correct_password", "new_password": "new_password"}
    ...
    assert (
        real_verify_password(user_payload["new_password"], user_db.hashed_pass) is True
    )
    assert (
        real_verify_password(user_payload["old_password"], user_db.hashed_pass) is False
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            False,
            404,
            {"old_password": "correct_password", "new_password": "new_correct_pass"},
            "",
            "",
        ),
        ...
        (
            True,
            422,
            {"new_password": "", "old_password": "correct_password"},
            "new_password",
            "string_too_short",
        ),
        ...
        (
            True,
            422,
            {"new_password": "p" * 66, "old_password": "correct_password"},
            "new_password",
            "string_too_long",
        ),
        ...
        (
            True,
            422,
            {"new_password": "correct_password", "old_password": ""},
            "old_password",
            "string_too_short",
        ),
        ...
        (
            True,
            422,
            {"new_password": "correct_password", "old_password": "p" * 65},
            "old_password",
            "string_too_long",
        ),
        ...
        (
            True,
            400,
            {"new_password": "correct_password", "old_password": "corr_pass"},
            "old_password",
            "string_too_long",
        ),
    ),
...
async def test_patch_user_invalid_payload_422_404(
    ...
    if expected_status == 422:
        error = response.json()["detail"][0]
        assert (
            expected_error_loc in error["loc"] and expected_error_type == error["type"]
        )
