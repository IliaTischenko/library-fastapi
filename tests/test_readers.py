...
async def fixed_readers(
    get_test_db_session: AsyncSession, fixed_books: list[Book], clear_readers: None
) -> list[Reader]:
    usernames = ["username1", "username2", "username3"]
    users = []
    for username in usernames:
        user_reader = User(
            username=username, hashed_pass="12345678", role=UserRole.READER
        )
    ...
    query = (
        select(Reader)
        .where(Reader.id.in_([reader.id for reader in readers]))
        .options(
...
async def test_get_readers_filtered_200(
    ...
    list_response_validate = [
        ReaderResponse.model_validate(res) for res in response.json()
    ]
    ...
    expected_readers_from_db = [
        r for r in fixed_readers if r.full_name in expected_reader_names
    ]
...
@pytest.mark.asyncio
async def test_get_reader_detail_200(client: AsyncClient, fixed_readers: list[Reader]):
...
@pytest.mark.asyncio
async def test_get_reader_detail_not_found_404(client: AsyncClient):
...
async def test_put_reader_as_user_no_owner_403(
    ...
    sorted_books_ids_from_existing_readers = sorted(
        [book.id for book in existing_readers.books]
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"full_name": "new_reader_name", "books_indices": []}),
    ],
)
@pytest.mark.asyncio
async def test_put_reader_success_200(
    ...
    actual_data = response_validate.model_dump(
        include={"full_name", "register_date"}, mode="json"
    )
...
@pytest.mark.parametrize(
    ...
    [
        # full_name <1
        (
            {"full_name": "", "books_ids": [1, 2], "register_date": "1888-10-03"},
            422,
            "full_name",
            "string_too_short",
        ),
        # full_name >30
        (
            {"full_name": "a" * 31, "books_ids": [1, 2], "register_date": "1888-10-03"},
            422,
            "full_name",
            "string_too_long",
        ),
        ...
        (
            {"books_ids": [1, 2], "register_date": "1888-10-03"},
            422,
            "full_name",
            "missing",
        ),
        ...
        (
            {"full_name": "correct_name", "register_date": "1888-10-03"},
            422,
            "books_ids",
            "missing",
        ),
        ...
        (
            {"full_name": "correct_name", "books_ids": [1, 2], "register_date": ""},
            422,
            "register_date",
            "date_from_datetime_parsing",
        ),
...
async def test_patch_reader_success_200(
    ...
    for key in patch_payload_pre.keys():
        if key == "books_indices":
            real_books_ids = sorted(
                [
                    int(fixed_books[int(idx)].id)
                    for idx in patch_payload_pre["books_indices"]
                ]
            )
    ...
    if patch_payload.get("register_date"):
        assert (
            patch_payload.get("register_date")
            == validate_response.register_date.isoformat()
        )
...
async def test_add_book_to_reader_success_200(
    ...
    query = (
        select(Reader)
        .where(Reader.id == target_reader_id)
        .options(selectinload(Reader.books))
    )
    result = await get_test_db_session.execute(query)
    db_reader = result.scalar_one_or_none()
...
async def test_delete_book_from_reader_success_200(
    ...
    response = await client.delete(
        f"/readers/{target_reader_id}/books/{target_book_id}"
    )
