...
@pytest.mark.asyncio
async def test_login_success_200(client: AsyncClient, fixed_users: list[User]):
    login_payload = {"username": "username1", "password": "correct_password"}
...
async def test_login_invalid_payloads_400(
    client: AsyncClient, fixed_users: list[User], invalid_payload: dict[str]
):
    client.cookies.delete("access_token")
...
@pytest.mark.asyncio
async def test_logout_200(client: AsyncClient, setup_auth):
    auth_behavior = {
        "id": 1,
        "role": "reader",
...
async def test_expire_invalid_token_401(
    client: AsyncClient, type_error: jwt.exceptions, error_message: str
):
...
@pytest.mark.asyncio
async def test_token_is_blacklisted_401(client: AsyncClient):
    client.cookies.set("access_token", "some_fake_token_string")
    with patch(
        "app.auth_utils.is_token_blacklisted", new_callable=AsyncMock
    ) as mock_blacklist_check:
        ...
        assert (
            ...
            == "Session revoked. Please log in again. (Token is blacklisted)"
        )
